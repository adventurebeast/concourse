...
**Identity:** `src/main/terminal-process.js` queries executable accounting names and numeric process/parent/group IDs for Concourse-owned PTY devices. On macOS it requests `ucomm`; on Linux it requests `comm`. It never requests arguments, environment variables, shell history, or node-pty's title/process getter. One bounded query runs every 1.5 seconds across the app's panes, and only changed results are sent to their owning windows. Selection stays within the owning terminal and foreground group. A recognized agent that owns the foreground job keeps its identity when it starts another agent as a helper. Shell and known runtime/package wrappers are followed through parent links to their outermost agent; ambiguous agent branches use a neutral label. Other applications retain ownership of their helpers. Process-list order cannot decide the provider.
