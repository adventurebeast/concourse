...
// Watchdog · Layer B — turn a pane's recent visible output into a compact
...
//   CONCOURSE_WATCHDOG_BASE_URL set  -> "local": any OpenAI-compatible HTTP endpoint
...
// the watchdog must never take the app down.
...
function claudeProvider(apiKey) {
  const model = (process.env.CONCOURSE_WATCHDOG_MODEL || '').trim() || DEFAULT_CLAUDE_MODEL
  ...
  const getClient = () => {
    if (!clientPromise) {
      clientPromise = import('@anthropic-ai/sdk')
        ...
        .catch((err) => {
          console.log('[watchdog] SDK unavailable:', err?.message || err)
          return null
        })
    }
...
// ignore Authorization; a key is sent only if CONCOURSE_WATCHDOG_API_KEY is set).
function openAICompatibleProvider() {
  const baseUrl = ((process.env.CONCOURSE_WATCHDOG_BASE_URL || '').trim() || DEFAULT_LOCAL_BASE_URL).replace(
    /\/+$/,
    ''
  )
  const model = (process.env.CONCOURSE_WATCHDOG_MODEL || '').trim() || DEFAULT_LOCAL_MODEL
  const key = (process.env.CONCOURSE_WATCHDOG_API_KEY || '').trim()
...
function createProvider() {
  if ((process.env.CONCOURSE_WATCHDOG_BASE_URL || '').trim()) return openAICompatibleProvider()
...
export function registerWatchdog() {
  ...
  ipcMain.handle('watchdog:summarize', async (_e, payload = {}) => {
    ...
    try {
      ...
      console.log('[watchdog] summarize failed:', err?.status || '', err?.message || err)
      return null
    } finally {
